...
export class SignupPage implements OnInit {
 ...
 createEntry(username : string,  email : string, password : string, gender: string, mobno :string) : void
 {
    let headers 	: any		= new HttpHeaders({ 'Content-Type': 'application/json' }),
        options 	: any		= { "key" : "create", "username" : username,"email" : email, "password" : password,"gender":gender,"mobno":mobno },
 ...
 updateEntry(username : string,  email : string, password : string,gender: string, mobno:string) : void
   {
      let headers 	: any		= new HttpHeaders({ 'Content-Type': 'application/json' }),
          options 	: any		= { "key" : "update", "username" : username, "recordID" : this.recordID,"email" : email, "password" : password,"gender":gender },
          url       : any      	= this.baseURI + "manage-data.php";
   ...
   signup1(){
    ...
    let mobno       : any = this.user.controls["mobno"].value;

if(this.isEdited)
{
   this.updateEntry(username, email,password,gender,mobno);
}
else
{
   this.createEntry(username, email,password,gender,mobno);
}
   }
   resetFields() : void
   {
